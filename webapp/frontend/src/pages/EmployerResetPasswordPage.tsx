...
/**
 * Where the link in an Employer's "Reset your password" mail lands. Same
 * shape as ResetPasswordPage.tsx (Seeker) — see that file's docstring for
 * why a missing/spent/expired token are all the same "invalid-link" outcome.
 */
export default function EmployerResetPasswordPage() {
  ...
  if (status === 'invalid-link') {
    return (
      <AuthShell eyebrow="Employer account" title="Link no longer valid">
        <div
  ...
  return (
    <AuthShell eyebrow="Employer account" title="Choose a new password">
      <div
      ...
      >
        <form onSubmit={handleSubmit} noValidate>
          ...
          <div className="mt-5">
            <AuthField label="Confirm new password" htmlFor="erp-confirm">
              <input
                id="erp-confirm" name="confirm_password" type="password" required
                minLength={MIN_PASSWORD} maxLength={MAX_PASSWORD}
                autoComplete="new-password" className="finex-input"
              />
            </AuthField>
