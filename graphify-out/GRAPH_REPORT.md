...
## Corpus Check
- 200 files · ~382,761 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2862 nodes · 5265 edges · 166 communities (139 shown, 27 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 443 edges (avg confidence: 0.64)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `7540bea8`
...
Cohesion: 0.11
Nodes (18): _efc_signals(), EfcAdapter, _map_employment_type(), _parse_iso(), eFinancialCareers adapter (per-company, JSON API).  ╔═══════════════════════════, Fetch one API page; return (job_dicts, total_page_count). Retries throttles., P2/P3 market signals from one eFC job object (only non-empty ones)., Parse an ISO 8601 timestamp (e.g. '2026-07-14T10:15:01.567Z') to aware UTC. (+10 more)

### Community 4 - "Daily Pipeline Orchestration"
Cohesion: 0.07
Nodes (56): backup_database(), Database backup utility.  Creates a dated copy of jobs.db in data/backups/. By d, migrate_to_phase_11(), migrate_to_phase_12(), migrate_to_phase_13(), migrate_to_phase_14(), migrate_to_phase_15(), migrate_to_phase_16() (+48 more)
...
Cohesion: 0.14
Nodes (12): EightfoldAdapter, Fetches jobs from Eightfold AI's public JSON API.      'tenant' is the subdomain, Public entry point — wraps _fetch_all in _safe_fetch for error isolation., Client, Job, main(), Live smoke-test for the Eightfold adapter.  Run this LOCALLY (not in CI) to conf, adapter() (+4 more)

### Community 7 - "Job Card & Detail Modal UI"
Cohesion: 0.10
Nodes (25): Job, JobDetail, LinkedInPostSignals, CardFooter(), CardHeader(), JobCard(), MetaRow(), Props (+17 more)
...
Cohesion: 0.08
Nodes (18): CoverageRadar(), DataFlow(), DATAFLOW_OUTS, DATAFLOW_SOURCES, GROWTH_BAR_HEIGHTS, GrowthBars(), SkylineTrend(), SvgProps (+10 more)
...
Cohesion: 0.12
Nodes (28): ApifyClient, Any, _best_email(), _is_fresh(), Pick the best candidate from the actor's `emails` array: prefer     deliverable=, run_email_harvest(), ApifyClient, Thin wrapper around the two HarvestAPI actors. Retries transient     failures (t (+20 more)
...
Cohesion: 0.10
Nodes (35): Any, ApifyClient, One validated entry from recruiters.yaml., RecruiterConfig, _backfill_promoted_jobs(), EmailHarvestSummary, LP-5 recruiter email harvest (PLAN_LINKEDIN_POSTS.md decision #10): a one-time-p, Push every recruiter's current harvested email into board_signals for     alread (+27 more)

### Community 20 - "Job Expiry & Deactivation"
Cohesion: 0.15
Nodes (17): JobStore, Manages a SQLite database of job postings.      Usage:         store = JobStore(, Close the underlying database connection., deactivate_stale_jobs(), Age-based expiry for Secret Market jobs.  Why this exists: --fetch-posts-backfil, Soft-delete active linkedin_posts jobs whose posted_at is older than     max_age, db(), _post_job() (+9 more)

### Community 21 - "Architecture & Market Panel Docs"
Cohesion: 0.19
Nodes (16): _extract_efc_id(), Extract the trailing `.id{NUMBER}` from a detailsPageUrl; fall back to url., _adapter(), _api_job(), _DummyClient, Tests for the eFinancialCareers adapter (JSON API).  The HTTP layer is mocked vi, A global brand returns worldwide jobs; only Hong Kong ones are kept., pages: list of (data_list, page_count) returned successively by _fetch_page. (+8 more)
...
Cohesion: 0.08
Nodes (27): Evidential Status Grading, Seven-Week HK Finance Posting Panel, Stable-Roster Window (16–25 July), _cluster_by_title(), _job_to_row(), _preferred_apply_url(), _primary_rowid(), Any (+19 more)
...
Cohesion: 0.09
Nodes (27): CompanyConfig, CompanyConfig, load_companies(), Company configuration loader.  Reads companies.yaml and returns validated Compan, Raise ValueError with the company name if the config is invalid., One validated entry from companies.yaml., Instantiate and return the adapter for this company.          Returns a BaseAdap, Parse companies.yaml and return a list of validated CompanyConfig objects. (+19 more)
...
Cohesion: 0.09
Nodes (14): countActiveFilters(), filtersToSearchParams(), JobListResponse, searchParamsToFilters(), TierTab, Props, Pagination(), Props (+6 more)
...
Cohesion: 0.08
Nodes (51): consume_email_token(), generate_token(), hash_token(), issue_email_token(), issue_session(), _matches(), Seeker authentication core — passwords, sessions, single-use email tokens, provi, A fresh opaque token. The only place raw token material is created. (+43 more)
...
Cohesion: 0.14
Nodes (11): LINKS, Nav(), Props, EMPLOYMENT_TYPES, MAX, Status, navClearancePx(), prefersReducedMotion() (+3 more)
...
Cohesion: 0.12
Nodes (18): CompanyResult, date, Write one job_history row per company and refresh company_metrics.      Called a, record_scrape_snapshot(), CompanyResult, _log_trend_changes(), _NullLock, _print_report() (+10 more)
...
Cohesion: 0.24
Nodes (10): Eightfold canonicalPositionUrl apply link, Eightfold locations/country/city triple, Eightfold positions array envelope, Eightfold t_create epoch-millis posted date, Workday bulletFields location list, Workday externalPath detail key, Workday jobPostings envelope, Workday jobReqId external identifier (+2 more)

### Community 80 - "LinkedIn Posts Phase Log"
Cohesion: 0.17
Nodes (13): 5. Phases, Age-based expiry, --deactivate-stale-posts (default 90 days), 48h date-filter bug: --fetch-posts-backfill added, LP-6 Steady state (post-GO), LP-0 — Vendor bake-off — ✅ COMPLETE (2026-07-19, cost $0.21), LP-1 — Watchlist bootstrap (~2 days, parallel with boss's manual list), LP-2 — Ingestion backbone — ✅ BUILT (2026-07-22), pending your review, LP-3 — Extraction & promotion (~2–3 days) (+5 more)
...
Cohesion: 0.43
Nodes (6): DEFAULT_FILTERS, diversify(), employerKey(), pickSector(), RecommendedRoles(), shuffled()
...
Cohesion: 0.33
Nodes (3): CAREER_STAGES, MAX, Status
...
Cohesion: 0.11
Nodes (32): ApiError, apiFetch(), authError(), deleteAccount(), EnquiryPayload, fetchFilters(), fetchJobDetail(), fetchJobs() (+24 more)
...
Cohesion: 0.10
Nodes (23): Adapter Abstraction, Canonical Job Model, Sector is Computed, Not Stored, Cross-Posted Jobs Share Board Signals, Read-Only FastAPI Layer, Filter to URL Serialisation, Graceful Per-Company Failure (_safe_fetch), Soft Delete & Cross-Post Reconciliation (+15 more)
...
Cohesion: 0.13
Nodes (20): Tests for the Seeker account HTTP surface (phase 2 of docs/PLAN_ACCOUNTS.md).  T, A Role that has left jobs.db entirely must not break the whole list., Decision 14: first sign-in lifts localStorage saves in, losing nothing., _register(), test_a_reference_to_a_vanished_role_is_skipped_not_a_500(), test_delete_account_really_deletes(), test_email_is_normalised(), test_honeypot_looks_like_success_but_creates_nothing() (+12 more)

### Community 125 - "Community 125"
Cohesion: 0.08
Nodes (19): Tests for the single-origin setup — FastAPI serving the built React bundle.  The, The failure this guards against: a retired or typo'd endpoint answering     200, React Router owns everything below a route too — a nested or future path     mus, index.html names the current hashed bundle. A cached copy pins a returning     v, favicon.svg and friends live at the root of the bundle, not under assets/., A missing hashed file must fail honestly. Handing back index.html would     make, ..-escapes resolve outside dist/ and fall through to the SPA, never to a     fil, Backend-only development, and a deploy whose frontend build failed: a     workin (+11 more)
...
Cohesion: 0.11
Nodes (22): Fallback Sources Carry 96% of Coverage (contradicts the ATS table), daily_run.sh 'all 27 companies' Comment Is Stale, AI enrichment output, Applicant Tracking System (ATS), Core domain knowledge (read this — it drives every design decision), Eightfold AI ATS, iCIMS ATS (hostile), Indeed employer-page fallback (GS/JPM/DBS) (+14 more)
...
Cohesion: 0.23
Nodes (13): _current_seeker(), list_saved(), merge_saved(), MergeSavedIn, The signed-in Seeker's row, or None. Never raises — anonymous is a normal     st, The Seeker's saved Roles, resolved against jobs.db at read time.      seekers.db, 204 whether or not it was saved — unsaving twice is not an error., Lift the browser's localStorage saves into the account on first sign-in     (dec (+5 more)
...
Cohesion: 0.15
Nodes (14): Apify $30/mo Cap Confirmed as a Hard Pre-Call Refusal, Adapter abstraction pattern (central abstraction), Architecture (the shape of the solution), CLAUDE.md — HK Financial Job Scraper, Exception: LinkedIn recruiter posts ("Secret Market"), HK Financial Job Scraper (CLAUDE.md project brief), LinkedIn recruiter-posts paid-vendor exception ($30/mo cap), Project conventions (+6 more)

### Community 151 - "Community 151"
Cohesion: 0.25
Nodes (8): hash_password(), password_needs_rehash(), Hash a plaintext password with Argon2id.      The returned string is self-descri, True when a stored hash was produced with weaker parameters than current.      C, Per-hash salt — two Seekers with the same password must not look alike., seeker_id(), test_needs_rehash_is_false_for_a_fresh_hash(), test_the_same_password_hashes_differently_each_time()

### Community 152 - "Community 152"
Cohesion: 0.25
Nodes (8): Check a password against a stored hash. Never raises; returns a bool.      `pass, _safe_verify(), verify_password(), A Google-only Seeker has no password. Verifying against None must simply fail., test_corrupt_hash_never_authenticates(), test_missing_hash_is_a_failed_login_not_a_crash(), test_password_round_trips(), test_wrong_password_fails()
...
- **Why does `Job` connect `Job Schema & JSONL I/O` to `Enrichment Pipeline & Tests`, `Longtail LLM Adapter`, `eFinancialCareers Adapter Tests`, `Eightfold Adapter & Tests`, `Community 135`, `Base Adapter Abstraction`, `Pipeline Run Tests`, `Job Expiry & Deactivation`, `Cross-Posting Match Tests`, `JobsDB Adapter Core`, `Storage Cross-Post Reconciliation`, `Workday Adapter`, `Post Promotion & Extraction Types`, `Company Config Loader`, `Ghost-Job Verification`, `Post Promotion Tests`, `Storage Layer Tests`, `Indeed Adapter`, `LinkedIn Guest Jobs Adapter`, `Workday & Eightfold Listing Fixtures`, `Indeed Listing JSON Parsing`, `Landing Brand Identity`, `Workday Job Detail Fixtures`, `CLAUDE.md Project Brief`, `LinkedIn Date & URL Parsing`, `Project Brief & Paid-Vendor Exception`?**
