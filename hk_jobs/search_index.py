...
def to_match_query(conn: sqlite3.Connection, query: str) -> str:
    ...
    while i < len(tokens):
        ...
        if alternatives is not None:
            # A company-group alias is evidence about the employer, not a
            # keyword that may occur anywhere in a job description.  In
            # particular, ``"ey"*`` used to match prose such as
            # ``"eye-opening"`` and made a Big Four search return unrelated
            # employers.  Keep every term literal and scope it to the company
            # column; legal-name variants are explicit in the registry above.
            options = [
...
def _primary_match_query(conn: sqlite3.Connection, query: str, match: str) -> str:
    """
    Restrict an ordinary multi-word query to one high-intent field.

    Full descriptions remain valuable recall evidence for a single skill or
    topic.  For a role-shaped query such as ``financial manager``, however,
    letting its terms land anywhere in a long description admits jobs which
    merely work in financial services and happen to manage something.  When
    the catalogue has a title, translated-title, or employer match containing
    every term, that is the intent a Seeker expressed.

    Employer-group aliases already scope themselves to ``company`` and must
    keep their own semantics, so they deliberately bypass this gate.
    """
    tokens = [t for t in _TOKEN.split((query or "").lower()) if t]
...
def _exact_title_rowids(conn: sqlite3.Connection, query: str) -> set[int]:
    ...
    BM25 is deliberately frequency-sensitive, which is usually useful.  It
    does mean a title that repeats ``financial manager`` four times can outrank
    the clean title ``Financial Manager``.  A Seeker who supplies a complete
    title has supplied stronger intent than that term frequency, so exact title
    matches receive one small, deterministic promotion.
    """
    tokens = [t for t in _TOKEN.split((query or "").lower()) if t]
    if len(tokens) < 2 or _has_company_group_alias(tokens):
        return set()
...
def _exclude_opposite_title_matches(
    ...
    tokens = [t for t in _TOKEN.split((query or "").lower()) if t]
    # FTS tokenises ``non-financial`` as ``non financial``.  A user searching
    # financial roles therefore should not receive a title explicitly saying it
    # is non-financial; when they do search for the latter, ``non`` opts out.
